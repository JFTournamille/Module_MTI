...
// ── 28. Emplacements de stockage : un référentiel de places ──
console.log('\n28. Emplacements de stockage')
{
  ...
  r = await j('GET', `/api/contenants/${cuve.id}/emplacements?enService=oui`)
  const places = r.corps
  places.length === 10 && places[0].libelle === `${code}-A-01`
    ? ok(`dix places en service, nommées « ${places[0].libelle} »`)
    : ko(`${places.length} place(s), première « ${places[0]?.libelle} »`)

  /* IL N'Y A PLUS DE RÉSERVATION — décision du 18 septembre. Ni disponibilité,
     ni délai. Ce groupe le VÉRIFIE plutôt que de le supposer : si l'une des
     deux revenait sans que le périmètre ait été rediscuté, c'est ici que ça se
     verrait. */
  places.every((p) => p.occupation === undefined)
    ? ok('une place ne porte aucune occupation')
    : ko('une place porte encore une occupation')

  r = await j('GET', '/api/contenants')
  const listee = r.corps.find((c) => c.id === cuve.id)
  listee && listee.nbLibres === undefined && listee.nbOccupees === undefined &&
  listee.nbActives === 10
    ? ok('le contenant compte ses places et celles en service, rien de « libre »')
    : ko(`contenant : ${JSON.stringify(listee)}`)

  r = await j('GET', '/api/parametres')
  r.corps.every((p) => p.cle !== 'emplacement.expiration_heures')
    ? ok('aucun délai de réservation dans les paramètres')
    : ko('le délai de réservation est toujours réglable')

  for (const url of ['/api/emplacements/x/reserver', '/api/occupations/x/liberer']) {
    r = await j('POST', url, {})
    r.statut === 404
      ? ok(`la route ${url} n'existe plus`)
      : ko(`${url} répond ${r.statut}`)
  }

  // ── Ce que la saisie fait, et qui suffit ──
  r = await j('POST', '/api/dossiers',
  ...
  /* Le point est cherché DANS CHAQUE dossier : deux dossiers du même parcours
     ont les mêmes points, mais pas les mêmes identifiants de processus. */
  const situerEmplacement = async (dossierId) => {
    const x = await j('GET', `/api/dossiers/${dossierId}`)
    for (const p of x.corps.processus) {
      const sections = p.definition?.sections ?? []
      for (let iS = 0; iS < sections.length; iS++) {
        const points = sections[iS].points ?? []
        for (let iP = 0; iP < points.length; iP++) {
  ...
  const cible = await situerEmplacement(dosB)

  if (!cible) {
    console.log('  · aucun point « emplacement » au parcours — saisie non éprouvée')
  } else {
    const saisir = (pid, place) => j('PUT', `/api/processus/${pid}/saisies`, {
      saisies: [{
    ...
    r = await saisir(cible.pid, places[1].id)
    r.statut === 200 ? ok('la saisie enregistre la place, sans rien réserver')
      : ko(`statut ${r.statut} : ${JSON.stringify(r.corps)}`)

    /* CE QUE CE PÉRIMÈTRE COÛTE, et qui est assumé : deux dossiers peuvent
       désigner la même cassette. Le module CONSTATE ce que l'opérateur a fait,
       il ne pilote pas l'occupation de la cuve. Si ce test échoue un jour, une
       contrainte d'unicité a été rétablie — vérifier que c'était voulu. */
    const cibleA = await situerEmplacement(dosA)
    r = await saisir(cibleA.pid, places[1].id)
    r.statut === 200
      ? ok('deux dossiers peuvent désigner la même place : périmètre voulu')
      : ko(`la collision est refusée (${r.statut}) — le périmètre a changé`)

    /* Le relevé fait foi : l'export doit imprimer le LIBELLÉ de la place, pas
       son identifiant technique, qui n'apprendrait rien à un lecteur. */
    const rep = await fetch(`${base}/api/dossiers/${dosB}/export.pdf`)
    const texte = Buffer.from(await rep.arrayBuffer()).toString('latin1')
    texte.includes(`${code}-A-02`)
      ? ok('l\'export imprime le libellé de la place, pas son identifiant')
      : ko('la place n\'apparaît pas lisiblement dans le document')
  }

  // Une place hors service dit pourquoi, et n'est plus proposée.
  ...
  r.statut === 200 ? ok('place sortie du service, avec son motif') : ko(`statut ${r.statut}`)
  r = await j('GET', `/api/contenants/${cuve.id}/emplacements?enService=oui`)
  ...
  r.statut === 200 && r.corps?.actif === false
    ? ok('un contenant se désactive — les dossiers qui le citent restent lisibles')
    : ko(`statut ${r.statut}`)
}
